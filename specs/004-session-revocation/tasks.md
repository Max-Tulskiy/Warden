...
**Plan:** [./plan.md](./plan.md) · **Status:** draft · **Date:** 2026-09-24
...
- [ ] T-22. Mirror T-21 in `docs/en/architecture.md`, same headings and same statements
- [ ] T-23. Update `docs/ru/usage.md`: the Settings paragraph now describes both actions and what each does to other sessions and to the current one; a note that operators sign in once after the upgrade; the honest-status section extended with what the manual pass did and did not cover
- [ ] T-24. Mirror T-23 in `docs/en/usage.md`

## Verification

- [ ] T-25. Run `ruff check .`, `ruff format --check .`, `mypy src alembic`, and `bandit -c pyproject.toml -r src` in `server/` — all clean
- [ ] T-26. Run the full server suite with `pytest --cov=warden_server --cov-report=xml --cov-fail-under=80` and, once the change is committed, `diff-cover coverage.xml --compare-branch=main --fail-under=80` — green, changed-code coverage at least 80%
- [ ] T-27. Run `ruff check .`, `ruff format --check .`, `mypy src`, `bandit -c pyproject.toml -r src`, and `pytest` in `agent/` — all clean and green (no source change; regression run)
- [ ] T-28. Run `npm run lint`, `npm run format`, `npm test`, and `npm run build` in `web/` — all clean
- [ ] T-29. Against real PostgreSQL from `docker compose` (a separate project with fresh volumes and the proxy on non-standard ports, without editing `docker-compose.yml`) run `alembic upgrade head`, `alembic downgrade -1`, `alembic upgrade head`, and confirm an operator row created before the upgrade reads `token_version` 0
- [ ] T-30. Manual pass on that stack, through the proxy and in a real browser: two browsers signed in; a password change in one; the other refused and returned to the sign-in screen with the message; the first still signed in; «Завершить все сеансы»; a token from before the upgrade refused after it; the audit screen shows the new actions under their Russian names; record the outcome and its limits in the honest-status section of `docs/ru/usage.md` and `docs/en/usage.md`
- [ ] T-31. Repository checks: no `sys.platform`/`platform.system` or platform imports in the touched server files (principle 1); `docs/ru` and `docs/en` have matching headings for the pages changed; everything added under `specs/` is English, apart from UI strings and Russian document headings quoted verbatim to identify them
- [ ] T-32. Set **Status** to `done` in `spec.md`, `plan.md`, and this file
